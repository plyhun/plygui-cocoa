...
impl ControlInner for CocoaTable {
    ...
    fn on_removed_from_container(&mut self, _: &mut MemberBase, _: &mut ControlBase, _: &dyn controls::Container) {
        ...
        let ll2: &Table = unsafe { common::member_from_cocoa_id(self.base.control).unwrap() };
...
impl Drop for CocoaTable {
    fn drop(&mut self) {
        let ll: &Table = unsafe { common::member_from_cocoa_id(self.base.control).unwrap() };
        for ref mut child in self.items.as_mut_slice() {
...
extern "C" fn get_item_height(this: &mut Object, _: Sel, _: cocoa_id, row: NSInteger) -> f64 {
    let sp = unsafe { common::member_from_cocoa_id::<Table>(this).unwrap() };
